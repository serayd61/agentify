...
export async function middleware(request: NextRequest) {
  ...
  if (!supabaseUrl || !supabaseAnonKey) {
    // Only block protected routes if Supabase is not configured
  ...
  // Only run auth check for protected/auth routes
  ...
  try {
    ...
    // Redirect to dashboard if authenticated on auth route
    if (isAuthRoute && user) {
